...
# keyorDefault requires two values. The key and default. In these examples :9999 is the default value.
# The key value is pulled from env variable 
...
{{ define "custom" }}service/fabio/{{env "NOMAD_JOB_NAME" }}/listeners{{ end }}
   ...
   math - alloc_id + 1: {{env "NOMAD_ALLOC_INDEX" | parseInt | add 1}}
    ...
    concat key:     secret/static/alloc{{env "NOMAD_ALLOC_INDEX" }}
    key:            {{ keyOrDefault ( printf "secret/static/alloc%s" ( env "NOMAD_ALLOC_INDEX" ) ) ":9999" }}

# Long hand of concat key example above
{{ $jobName := env "NOMAD_JOB_NAME" }}
{{- $keyName :=  printf "service/fabio/%s/listeners" $jobName }}
{{- $value := keyOrDefault $keyName ":9999" }}
{{- $value }}

# Secret read from Vault. Vault/Nomad integration setup required
{{ with secret ( printf "secret/hello/%s/test" ( env "NOMAD_JOB_NAME" ) )  }}{{.Data.data.foo}}{{end}}
